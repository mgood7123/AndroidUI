...
namespace AndroidUI.Widgets
{
    ...
    public class ScrollView : FrameLayout
    {
         ...
         *
         * Even though this field is practically final, we cannot make it because there are apps
         ...
         */
        private /*StrictMode.Span*/object mScrollStrictSpan = null;  // aka "drag"
        private /*StrictMode.Span*/object mFlingStrictSpan = null;
        ...
        override
            public void addView(View child, View.LayoutParams layout_params)
        {
            if (getChildCount() > 0)
            {
                throw new IllegalStateException("ScrollView can host only one direct child");
            ...
            base.addView(child, layout_params);
        }

        override
            public void addView(View child, int index, View.LayoutParams layout_params)
        {
            if (getChildCount() > 0)
            {
                throw new IllegalStateException("ScrollView can host only one direct child");
            ...
            base.addView(child, index, layout_params);
        }
         ...
         * @attr ref android.R.styleable#ScrollView_fillViewport
         */
        public void setFillViewport(bool fillViewport)
        {
         ...
         */
        public void setSmoothScrollingEnabled(bool smoothScrollingEnabled)
        {
        ...
        {
            Log.WriteLine("INTERCEPT ScrollX = " + getScrollX() + ", ScrollY = " + getScrollY());
            /*
            ...
            */
            ...
            Touch.State action = touch.state;
            if ((action == Touch.State.TOUCH_MOVE) && (mIsBeingDragged))
            {
            ...
            {
                Console.WriteLine("return false, cannot scroll vertically and y is zero");
                return false;
            }
                ...
                case Touch.State.TOUCH_DOWN:
                    {
                        if (ev.touchCount == 1)
                        {
                            ...
                            */
                            ...
                            {
                                mEdgeGlowTop.onPullDistance(0f, touch.location.x / getWidth());
                            }
                            if (!mEdgeGlowBottom.isFinished())
                            {
                                mEdgeGlowBottom.onPullDistance(0f, 1f - touch.location.x / getWidth());
                            }
        ...
        private bool shouldDisplayEdgeEffects()
        {
            return getOverScrollMode() != OVERSCROLL_MODE.OVER_SCROLL_NEVER;
        }
            ...
            public bool onTouch(Touch ev)
        {
            ...
            if (actionMasked == Touch.State.TOUCH_DOWN)
            {
                mNestedYOffset = 0;
            }
            ...
            switch (actionMasked)
            {
                ...
                case Touch.State.TOUCH_MOVE:
                    ...
                    if (mIsBeingDragged)
                    {
                        ...
                        OVERSCROLL_MODE overscrollMode = getOverScrollMode();
                        ...
                        float displacement = touch.location.y / getWidth();
                        if (canOverscroll)
                        {
                            int consumed = 0;
                            if (deltaY < 0 && mEdgeGlowBottom.getDistance() != 0f)
                            {
                                consumed = (int)Math.Round(getHeight()
        ...
        //private void onSecondaryPointerUp(Touch ev)
        //{
        //    int pointerIndex = (ev.getAction() & Touch.State.TOUCH_POINTER_INDEX_MASK) >>
        //            Touch.State.TOUCH_POINTER_INDEX_SHIFT;
        ...
        //    int delta = Math.round(axisValue * mVerticalScrollFactor);
        //    if (delta != 0)
        //    {
        //        int range = getScrollRange();
        ...
        //            bool viewIsFullyContained = (top < viewTop) &&
        //                    (viewBottom < bottom);
         ...
         */
        public bool pageScroll(int direction)
        {
         ...
         * @return true if the key event is consumed by this method, false otherwise
         */
        public bool fullScroll(int direction)
        {
            bool down = direction == View.FOCUS_DOWN;
            int height = getHeight();
        ...
        private bool scrollAndFocus(int direction, int top, int bottom)
        {
            ...
            View newFocused = null;// findFocusableViewInBounds(up, top, bottom);
            if (newFocused == null)
            {
                newFocused = this;
            }
         ...
         */
        public bool arrowScroll(int direction)
        {
                    ...
                    && isOffScreen(currentFocused))
            {
                ...
                setDescendantFocusability(View.FOCUS_BEFORE_DESCENDANTS);
                requestFocus();
        ...
        public void smoothScrollBy(int dx, int dy)
        {
            ...
            long duration = AnimationUtils.currentAnimationTimeMillis(Context) - mLastScroll;
            ...
            mLastScroll = AnimationUtils.currentAnimationTimeMillis(Context);
        }
                    ...
                    int parentHeightMeasureSpec, int heightUsed)
        {
            ...
            int childWidthMeasureSpec = getChildMeasureSpec(parentWidthMeasureSpec,
                    mPaddingLeft + mPaddingRight + lp.leftMargin + lp.rightMargin
                            + widthUsed, lp.width);
            int usedTotal = mPaddingTop + mPaddingBottom + lp.topMargin + lp.bottomMargin +
                    heightUsed;
         ...
         */
        override
            protected bool onRequestFocusInDescendants(int direction,
        ...
        override
            public bool requestChildRectangleOnScreen(View child, Rect rectangle,
                    bool immediate)
        {
        ...
        private void flingWithNestedDispatch(int velocityY)
        {
            ...
            {
                bool consumed = dispatchNestedFling(0, velocityY, canFling);
                if (canFling)
                {
                    fling(velocityY);
                }
         ...
         */
        override
            public bool onNestedFling(View target, float velocityX, float velocityY, bool consumed)
        {
            if (!consumed)
            {
            ...
            public void draw(SkiaSharp.SKCanvas canvas)
        {
            ...
            if (shouldDisplayEdgeEffects())
            {
                ...
                if (!mEdgeGlowBottom.isFinished())
                {
                    ...
                    canvas.RotateDegrees(180, width, 0);
