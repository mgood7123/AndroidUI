...
namespace AndroidUI
{
    ...
    public abstract class ColorSpace {
         ...
         * @see #getId()
         */
        public const int MIN_ID = -1; // Do not change
        /**
         * The maximum ID value a color space can have.
         *
         * @see #getId()
         */
        public const int MAX_ID = 63; // Do not change, used to encode in longs

        private static readonly float[] SRGB_PRIMARIES = { 0.640f, 0.330f, 0.300f, 0.600f, 0.150f, 0.060f };
        private static readonly float[] NTSC_1953_PRIMARIES = { 0.67f, 0.33f, 0.21f, 0.71f, 0.14f, 0.08f };
        /**
         * A gray color space does not have meaningful primaries, so we use this arbitrary set.
         */
        private static readonly float[] GRAY_PRIMARIES = { 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f };
        ...
        public class Adaptation {
             ...
             */
            public static readonly Adaptation BRADFORD = new(new float[] {
             ...
             */
            public static readonly Adaptation VON_KRIES = new(new float[] {
             ...
             */
            public static readonly Adaptation CIECAT02 = new(new float[] {
                 0.7328f, -0.7036f,  0.0030f,
                 0.4296f,  1.6975f,  0.0136f,
                -0.1624f,  0.0061f,  0.9834f
        });

            internal readonly float[] mTransform;
        ...
        public class Model {
             ...
             * model.
             */
            public static readonly Model CMYK = new(4);

            private readonly int mComponentCount;
    ...
    int id) {
         ...
         *         false otherwise
         */
        public abstract bool isWideGamut();
        ...
        override public int GetHashCode() {
            int result = mName.GetHashCode();
            result = 31 * result + mModel.GetHashCode();
         ...
         */
        internal static ColorSpace get(int index) {
        ...
        public static ColorSpace match(
            ...
            foreach (ColorSpace colorSpace in sNamedColorSpaces) {
                if (colorSpace.getModel() == Model.RGB) {
        ...
        static ColorSpace() {
            ...
            sNamedColorSpaces[Named.EXTENDED_SRGB.Ordinal()] = new Rgb(
                    ...
                    ILLUMINANT_D65,
                    null,
                    new(x=>absRcpResponse(x, 1 / 1.055, 0.055 / 1.055, 1 / 12.92, 0.04045, 2.4)),
                    new(x=>absResponse(x, 1 / 1.055, 0.055 / 1.055, 1 / 12.92, 0.04045, 2.4)),
                    -0.799f, 2.399f,
                    SRGB_TRANSFER_PARAMETERS,
        ...
        private static double absRcpResponse(double x, double a, double b, double c, double d, double g) {
            return MathUtils.copySign(rcpResponse(x < 0.0 ? -x : x, a, b, c, d, g), x);
        }
        ...
        private static double absResponse(double x, double a, double b, double c, double d, double g) {
            return MathUtils.copySign(response(x < 0.0 ? -x : x, a, b, c, d, g), x);
        }
        ...
        private static bool compare(float[] a, float[] b) {
            ...
            for (int i = 0; i < a.Length; i++) {
                if (!ContainerHelpers.float_type_value_equals(a[i], b[i]) && Math.Abs(a[i] - b[i]) > 1e-3f) return false;
            }
            return true;
        }
         ...
         */
        private sealed class Xyz : ColorSpace {
            internal Xyz(string name, int id) : base(name, Model.XYZ, id)
            {
            }

            override public bool isWideGamut() {
                return true;
            }

            override public float getMinValue(int component) {
                return -2.0f;
            }

            override public float getMaxValue(int component) {
                return 2.0f;
            }

            override public float[] toXyz(float[] v) {
                v[0] = clamp(v[0]);
                v[1] = clamp(v[1]);
                v[2] = clamp(v[2]);
                return v;
            ...
            override public float[] fromXyz(float[] v) {
                v[0] = clamp(v[0]);
                v[1] = clamp(v[1]);
                v[2] = clamp(v[2]);
                return v;
         ...
         */
        private sealed class Lab : ColorSpace {
            private const float A = 216.0f / 24389.0f;
            private const float B = 841.0f / 108.0f;
            private const float C = 4.0f / 29.0f;
            private const float D = 6.0f / 29.0f;

            internal Lab(string name, int id) : base(name, Model.LAB, id)
            {
            }
         ...
         */
        internal virtual SkiaSharp.SKColorSpace getNativeInstance() {
        ...
        public class Rgb : ColorSpace {
            ...
            public class TransferParameters {
                ...
                public TransferParameters(double a, double b, double c, double d, double e,
                    ...
                    if (!(d >= 0.0 && d <= 1.0f + MathUtils.ulp(1.0f))) {
                ...
                override public bool Equals(object o) {
                    ...
                    if (!ContainerHelpers.double_type_value_equals(that.a, a)) return false;
                    if (!ContainerHelpers.double_type_value_equals(that.b, b)) return false;
                    if (!ContainerHelpers.double_type_value_equals(that.c, c)) return false;
                    if (!ContainerHelpers.double_type_value_equals(that.d, d)) return false;
                    if (!ContainerHelpers.double_type_value_equals(that.e, e)) return false;
                    if (!ContainerHelpers.double_type_value_equals(that.f, f)) return false;
                    return !ContainerHelpers.double_type_value_equals(that.g, g);
                }
            ...
            internal readonly float[] mWhitePoint;
            internal readonly float[] mPrimaries;
            internal readonly float[] mTransform;
            internal readonly float[] mInverseTransform;

            private readonly DoubleUnaryOperator mOetf;
            private readonly DoubleUnaryOperator mEotf;
            internal readonly DoubleUnaryOperator mClampedOetf;
            internal readonly DoubleUnaryOperator mClampedEotf;

            private readonly float mMin;
            private readonly float mMax;

            private readonly bool mIsWideGamut;
            private readonly bool mIsSrgb;

            internal readonly TransferParameters mTransferParameters;
            private readonly SkiaSharp.SKColorSpace mNativePtr;

            internal override SkiaSharp.SKColorSpace getNativeInstance() {
                if (mNativePtr == null) {
            ...
            private static SkiaSharp.SKColorSpace nativeCreate(float a, float b, float c, float d,
                    float e, float f, float g, float[] xyzD50)
            {
                SkiaSharp.SKColorSpaceTransferFn p = new();
                p.A = a;
                p.B = b;
                p.C = c;
                p.D = d;
            ...
            internal Rgb(
                ...
                this(name, primaries, whitePoint, transform,
                        function.e == 0.0 && function.f == 0.0 ?
                                new(x => rcpResponse(x, function.a, function.b,
                                        function.c, function.d, function.g)) :
                                new(x => rcpResponse(x, function.a, function.b, function.c,
                                        function.d, function.e, function.f, function.g)),
                        function.e == 0.0 && function.f == 0.0 ?
                                new(x => response(x, function.a, function.b,
                                        function.c, function.d, function.g)) :
                                new(x => response(x, function.a, function.b, function.c,
                                        function.d, function.e, function.f, function.g)),
            ...
            internal Rgb(
                ...
                this(name, primaries, whitePoint, null,
                        gamma == 1.0 ? DoubleUnaryOperator.identity() :
                                new(x => Math.Pow(x < 0.0 ? 0.0 : x, 1 / gamma)),
                        gamma == 1.0 ? DoubleUnaryOperator.identity<DoubleUnaryOperator>() :
                                new(x => Math.Pow(x < 0.0 ? 0.0 : x, gamma)),
                ...
                base(name, Model.RGB, id)
            {
                ...
                DoubleUnaryOperator clamp = new(this.clamp);
                mClampedOetf = (DoubleUnaryOperator)oetf.andThen(clamp);
                mClampedEotf = (DoubleUnaryOperator)clamp.andThen(eotf);
            ...
            //    public static readonly NativeAllocationRegistry sRegistry = new NativeAllocationRegistry(
            //        ColorSpace.Rgb.class.GetTypeLoader(), nativeGetNativeFinalizer(), 0);
            //}
            ...
            public float[] getPrimaries(float[] primaries) {
                Arrays.arraycopy(mPrimaries, 0, primaries, 0, mPrimaries.Length);
                return primaries;
            }
            ...
            public float[] getTransform(float[] transform) {
                Arrays.arraycopy(mTransform, 0, transform, 0, mTransform.Length);
                return transform;
            }
            ...
            public float[] getInverseTransform(float[] inverseTransform) {
                Arrays.arraycopy(mInverseTransform, 0, inverseTransform, 0, mInverseTransform.Length);
            ...
            public float[] toLinear(float[] v) {
                v[0] = (float)mClampedEotf.apply(v[0]);
                v[1] = (float)mClampedEotf.apply(v[1]);
                v[2] = (float)mClampedEotf.apply(v[2]);
                return v;
            }
            ...
            public float[] fromLinear(float[] v) {
                v[0] = (float)mClampedOetf.apply(v[0]);
                v[1] = (float)mClampedOetf.apply(v[1]);
                v[2] = (float)mClampedOetf.apply(v[2]);
                return v;
            }
            ...
            override public bool Equals(object o) {
                ...
                if (rgb.mMin.CompareTo(mMin) != 0) return false;
                if (rgb.mMax.CompareTo(mMax) != 0) return false;
            ...
            override public int GetHashCode() {
                ...
                result = 31 * result + (mMin != +0.0f ? mMin.ToIntBits() : 0);
                result = 31 * result + (mMax != +0.0f ? mMax.ToIntBits() : 0);
                result = 31 * result +
                        (mTransferParameters != null ? mTransferParameters.GetHashCode() : 0);
                if (mTransferParameters == null) {
                    result = 31 * result + mOetf.GetHashCode();
                    result = 31 * result + mEotf.GetHashCode();
                }
                return result;
            }
             ...
             */
            private static bool isGray(float[] toXYZ) {
                return toXYZ.Length == 9 && toXYZ[1] == 0 && toXYZ[2] == 0 && toXYZ[3] == 0
                        && toXYZ[5] == 0 && toXYZ[6] == 0 && toXYZ[7] == 0;
            }

            private static bool compare(double point, DoubleUnaryOperator a,
                    DoubleUnaryOperator b) {
                double rA = a.apply(point);
                double rB = b.apply(point);
             ...
             * @see #area(float[])
             */
            private static bool isWideGamut(float[] primaries,
                    float min, float max) {
             ...
             * @see #isWideGamut(float[], float, float)
             */
            private static bool contains(float[] p1, float[] p2) {
            ...
            private static float[] xyPrimaries(float[] primaries) {
                ...
                if (primaries.Length == 9) {
                    ...
                    Arrays.arraycopy(primaries, 0, xyPrimaries, 0, 6);
                }
            ...
            private static float[] xyWhitePoint(float[] whitePoint) {
                ...
                if (whitePoint.Length == 3) {
                    ...
                    Arrays.arraycopy(whitePoint, 0, xyWhitePoint, 0, 2);
                }
        ...
        public class Connector {
            private readonly ColorSpace mSource;
            private readonly ColorSpace mDestination;
            private readonly ColorSpace mTransformSource;
            private readonly ColorSpace mTransformDestination;
            private readonly RenderIntent mIntent;
             ...
             */
            internal Connector(ColorSpace source, ColorSpace destination,
                    RenderIntent intent) :
            ...
            private static float[] computeTransform(ColorSpace source,
                ...
                bool srcRGB = source.getModel() == Model.RGB;
                bool dstRGB = destination.getModel() == Model.RGB;
            ...
            internal class Rgb : Connector {
                ...
                internal Rgb(ColorSpace.Rgb source, ColorSpace.Rgb destination,
                        RenderIntent intent) : base(source, destination, source, destination, intent, null) {
                    mSource = source;
                    mDestination = destination;
                    mTransform = computeTransform(source, destination, intent);
                }

                override public float[] transform(float[] rgb) {
                    rgb[0] = (float)mSource.mClampedEotf.apply(rgb[0]);
                    rgb[1] = (float)mSource.mClampedEotf.apply(rgb[1]);
                    rgb[2] = (float)mSource.mClampedEotf.apply(rgb[2]);
                    mul3x3Float3(mTransform, rgb);
                    rgb[0] = (float)mDestination.mClampedOetf.apply(rgb[0]);
                    rgb[1] = (float)mDestination.mClampedOetf.apply(rgb[1]);
                    rgb[2] = (float)mDestination.mClampedOetf.apply(rgb[2]);
                    return rgb;
                }
             ...
             * @see ColorSpace#connect(ColorSpace, ColorSpace)
             */
            internal static Connector identity(ColorSpace source) {
                return new IdentityConnectorNoTransform(source, source, RenderIntent.RELATIVE);
            }

            internal class IdentityConnectorNoTransform : Connector
            {
                internal IdentityConnectorNoTransform(ColorSpace source, ColorSpace destination, RenderIntent intent) : base(source, destination, intent)
                {
                }

                internal IdentityConnectorNoTransform(ColorSpace source, ColorSpace destination, ColorSpace transformSource, ColorSpace transformDestination, RenderIntent intent, float[] transform) : base(source, destination, transformSource, transformDestination, intent, transform)
                {
                }

                override public float[] transform(float[] v)
                {
                    return v;
                }
            }
