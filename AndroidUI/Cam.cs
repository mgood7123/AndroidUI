...
namespace AndroidUI
{
    ...
    public class Cam
    {
        ...
        public static Cam fromIntInFrame(int argb, Frame frame)
        {
            ...
            float rA = MathUtils.signum(rD) * 400.0f * rAF / (rAF + 27.13f);
            float gA = MathUtils.signum(gD) * 400.0f * gAF / (gAF + 27.13f);
            float bA = MathUtils.signum(bD) * 400.0f * bAF / (bAF + 27.13f);
