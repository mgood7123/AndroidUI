...
{
    /**
     * This class enables automatic animations on layout changes in View objects. To enable
     * transitions for a layout container, create a LayoutTransition object and set it on any
     * View by calling {@link View#setLayoutTransition(LayoutTransition)}. This will cause
    ...
    public class LayoutTransition
    {
         ...
         * mTransitionTypes.
         */
        private const int FLAG_APPEARING = 0x01;
        private const int FLAG_DISAPPEARING = 0x02;
        private const int FLAG_CHANGE_APPEARING = 0x04;
        private const int FLAG_CHANGE_DISAPPEARING = 0x08;
        private const int FLAG_CHANGING = 0x10;
        ...
        /**
         * These Dictionarys are used to store the animations that are currently running as part of
         ...
         * another layout event. Dictionarys are used to preserve the order in which animations
         * are inserted, so that we process events (such as setting up start values) in the same order.
         */
        private readonly Dictionary<View, Animator> pendingAnimations = new();
        private readonly Dictionary<View, Animator> currentChangingAnimations = new();
        private readonly Dictionary<View, Animator> currentAppearingAnimations = new();
        private readonly Dictionary<View, Animator> currentDisappearingAnimations = new();

        /**
         * This Dictionary is used to track the listeners that have been added to the children of
         ...
         */
        private Dictionary<View, View.OnLayoutChangeListener> layoutChangeListenerMap = new();
         ...
         * events on any View that it is set on and will run default animations for each
         * type of layout event.
         */
        public LayoutTransition(Context context)
        {
            ...
            if (defaultChangeIn == null)
            {
                ...
                defaultChangeIn = ObjectAnimator.ofPropertyValuesHolder(context, (object)null,
                ...
                defaultFadeIn = ObjectAnimator.ofFloat(context, null, "alpha", 0f, 1f);
                defaultFadeIn.setDuration(DEFAULT_DURATION);
                defaultFadeIn.setStartDelay(mAppearingDelay);
                defaultFadeIn.setInterpolator(mAppearingInterpolator);
                defaultFadeOut = ObjectAnimator.ofFloat(context, null, "alpha", 1f, 0f);
         ...
         */
        public bool isTransitionTypeEnabled(int transitionType)
        {
            switch (transitionType)
            {
                case APPEARING:
         ...
         */
        private void runChangeTransition(View parent, View newView, int changeReason)
        {
            ...
            if (mAnimateParentHierarchy)
            {
                ...
                while (tempParent != null)
                {
                    ...
                    if (parentParent is View) {
                        setupChangeAnimation((View)parentParent, changeReason, parentAnimator,
                                duration, tempParent);
                        tempParent = (View)parentParent;
                    } else
                    {
                        tempParent = null;
                    }
            ...
            // layout listeners.
            CleanupCallback callback = new(layoutChangeListenerMap, parent);
         ...
         *
         * @param animateParentHierarchy A bool value indicating whether the parents of
         * transitioning views should also be animated during the transition. Default value is true.
         */
        public void setAnimateParentHierarchy(bool animateParentHierarchy)
        {
        ...
        class AnimatorListenerAdapter1 : AnimatorListenerAdapter {

            LayoutTransition outer;
            View child;

            public AnimatorListenerAdapter1(LayoutTransition outer, View child)
            {
                this.outer = outer;
                this.child = child;
            }
            ...
            {
                outer.pendingAnimations.Remove(child);
            }
        }

        class AnimatorListenerAdapter2 : AnimatorListenerAdapter
        {
            ...
            public AnimatorListenerAdapter2(LayoutTransition outer, View child, View parent, int changeReason, View.OnLayoutChangeListener listener)
            {
                this.outer = outer;
                this.child = child;
                this.parent = parent;
            ...
            {
                if (outer.hasListeners())
                {
                    List<TransitionListener> listeners = new(outer.mListeners);
                    foreach (TransitionListener listener in listeners)
                    {
                        listener.startTransition(outer, parent, child,
            ...
            public void onAnimationCancel(Animator animator)
            {
                ...
                outer.layoutChangeListenerMap.Remove(child);
            }

            override
            public void onAnimationEnd(Animator animator)
            {
                outer.currentChangingAnimations.Remove(child);
                if (outer.hasListeners())
                {
                    List<TransitionListener> listeners = new(outer.mListeners);
                    foreach (TransitionListener listener in listeners)
                    {
                        listener.endTransition(outer, parent, child,
                                changeReason == APPEARING ?
                                        CHANGE_APPEARING : changeReason == DISAPPEARING ?
                                        CHANGE_DISAPPEARING : CHANGING);
                    }
        ...
        class OnLayoutChangeListener_ : View.OnLayoutChangeListener
        {
            ...
            public OnLayoutChangeListener_(LayoutTransition outer, View child, View parent, Animator anim, int changeReason, long duration)
            {
                this.outer = outer;
                this.child = child;
                this.parent = parent;
                    ...
                    int oldLeft, int oldTop, int oldRight, int oldBottom)
            {
                ...
                if (anim is ValueAnimator)
                {
                    ...
                    for (int i = 0; i < oldValues.Length; ++i)
                    {
                        ...
                        if (pvh.mKeyframes is KeyframeSet)
                        {
                ...
                {
                    case APPEARING:
                        startDelay = outer.mChangingAppearingDelay + outer.staggerDelay;
                        outer.staggerDelay += outer.mChangingAppearingStagger;
                        if (outer.mChangingAppearingInterpolator != sChangingAppearingInterpolator)
                        {
                            anim.setInterpolator((Interpolator)outer.mChangingAppearingInterpolator);
                        }
                        break;
                    case DISAPPEARING:
                        startDelay = outer.mChangingDisappearingDelay + outer.staggerDelay;
                        outer.staggerDelay += outer.mChangingDisappearingStagger;
                        if (outer.mChangingDisappearingInterpolator !=
                                sChangingDisappearingInterpolator)
                        {
                            anim.setInterpolator((Interpolator)outer.mChangingDisappearingInterpolator);
                        }
                        break;
                    case CHANGING:
                        startDelay = outer.mChangingDelay + outer.staggerDelay;
                        outer.staggerDelay += outer.mChangingStagger;
                        if (outer.mChangingInterpolator != sChangingInterpolator)
                        {
                            anim.setInterpolator((Interpolator)outer.mChangingInterpolator);
                        }
                        break;
                }
                ...
                Animator prevAnimation = outer.currentChangingAnimations[child];
                if (prevAnimation != null)
                {
                    prevAnimation.cancel();
                }
                Animator pendingAnimation = outer.pendingAnimations[child];
                if (pendingAnimation != null)
                {
                    outer.pendingAnimations.Remove(child);
                }
                // Cache the animation in case we need to cancel it later
                outer.currentChangingAnimations[child] = anim;

                parent.requestTransitionStart(outer);
        ...
        class AnimatorListenerAdapter3 : AnimatorListenerAdapter
        {
            ...
            public AnimatorListenerAdapter3(LayoutTransition outer, View child, View parent)
            {
                this.outer = outer;
                this.child = child;
                this.parent = parent;
            ...
            public void onAnimationEnd(Animator animation)
            {
                outer.currentAppearingAnimations.Remove(child);
                if (outer.hasListeners())
                {
                    List<TransitionListener> listeners = new(outer.mListeners);
                    foreach (TransitionListener listener in listeners)
                    {
                        listener.endTransition(outer, parent, child, APPEARING);
                    }
                }
            }
        }
        ...
        class AnimatorListenerAdapter4 : AnimatorListenerAdapter
        {
            ...
            public AnimatorListenerAdapter4(LayoutTransition outer, View child, View parent, float preAnimAlpha)
            {
                this.outer = outer;
                this.child = child;
                this.parent = parent;
                this.preAnimAlpha = preAnimAlpha;
            }

            override
            public void onAnimationEnd(Animator animation)
            {
                outer.currentDisappearingAnimations.Remove(child);
                ...
                if (outer.hasListeners())
                {
                    ...
                    foreach (TransitionListener listener in listeners)
                    {
                        listener.endTransition(outer, parent, child, DISAPPEARING);
                    }
                }
            }
        }
         ...
         * hierarchy.
         */
        private void setupChangeAnimation(View parent, int changeReason,
                Animator baseAnimator, long duration, View child)
        {
            ...
            if (layoutChangeListenerMap[child] != null)
            {
                return;
            }
            ...
            Animator currentAnimation = pendingAnimations[child];
            ...
            ValueAnimator pendingAnimRemover = ValueAnimator.ofFloat(context, 0f, 1f).
                    setDuration(duration + 100);
            pendingAnimRemover.addListener(new AnimatorListenerAdapter1(this, child));
            pendingAnimRemover.start();

            // Add a listener to track layout changes on this view. If we don't get a callback,
            // then there's nothing to animate.
            View.OnLayoutChangeListener listener = new OnLayoutChangeListener_(this, child, parent, anim, changeReason, duration);
            // Remove the animation from the cache when it ends
            anim.addListener(new AnimatorListenerAdapter2(this, child, parent, changeReason, listener));

            child.addOnLayoutChangeListener(listener);
            // cache the listener for later removal
            layoutChangeListenerMap[child] = listener;
        }
        ...
        {
            Dictionary<View, Animator> currentAnimCopy = new(currentChangingAnimations);
            foreach (Animator anim in currentAnimCopy.Values) {
        ...
        {
            return currentChangingAnimations.Count > 0;
        }
        ...
        {
            return currentChangingAnimations.Count > 0 || currentAppearingAnimations.Count > 0 ||
                    currentDisappearingAnimations.Count > 0;
        }
        ...
        public void cancel()
        {
            if (currentChangingAnimations.Count > 0)
            {
                Dictionary<View, Animator> currentAnimCopy = new(currentChangingAnimations);
                foreach (Animator anim in currentAnimCopy.Values) {
                    anim.cancel();
            ...
            if (currentAppearingAnimations.Count > 0)
            {
                Dictionary<View, Animator> currentAnimCopy = new(currentAppearingAnimations);
                foreach (Animator anim in currentAnimCopy.Values)
                {
                    anim.end();
                }
            ...
            if (currentDisappearingAnimations.Count > 0)
            {
                Dictionary<View, Animator> currentAnimCopy = new(currentDisappearingAnimations);
                foreach (Animator anim in currentAnimCopy.Values)
                {
                    anim.end();
                }
         ...
         *
         * @param parent The View to which the View is being added.
         * @param child The View being added to the View.
         */
        private void runAppearingTransition(View parent, View child)
        {
            Animator currentAnimation = currentDisappearingAnimations[child];
            if (currentAnimation != null)
            {
                currentAnimation.cancel();
            }
            if (mAppearingAnim == null)
            {
                if (hasListeners())
                {
                    List<TransitionListener> listeners = new(mListeners);
                    foreach (TransitionListener listener in listeners) {
            ...
            anim.addListener(new AnimatorListenerAdapter3(this, child, parent));
         ...
         *
         * @param parent The View from which the View is being removed.
         * @param child The View being removed from the View.
         */
        private void runDisappearingTransition(View parent, View child)
        {
            Animator currentAnimation = currentAppearingAnimations[child];
            if (currentAnimation != null)
            {
                currentAnimation.cancel();
            }
            ...
            anim.addListener(new AnimatorListenerAdapter4(this, child, parent, preAnimAlpha));
            if (anim is ObjectAnimator) {
                ((ObjectAnimator)anim).setCurrentPlayTime(0);
            }
        ...
        /**
         * This method is called by View when a child view is about to be added to the
         ...
         */
        private void addChild(View parent, View child, bool changesLayout)
        {
        ...
        {
            return mListeners != null && mListeners.Count() > 0;
        }

        /**
         * This method is called by View when there is a call to layout() on the container
         ...
         *
         * @param parent The View whose layout() method has been called.
         *
         * @hide
         */
        public void layoutChange(View parent)
        {
            if (parent.getWindowVisibility() != View.VISIBLE)
            {
                return;
        ...
        /**
         * @deprecated Use {@link #showChild(android.view.View, android.view.View, int)}.
         */
        [Obsolete]
        public void showChild(View parent, View child)
        {
            addChild(parent, child, true);
        }

        /**
         * This method is called by View when a child view is about to be made visible in the
         * container. This callback starts the process of a transition; we grab the starting
         * values, listen for changes to all of the children of the container, and start appropriate
         * animations.
         *
         * @param parent The View in which the View is being made visible.
         ...
         */
        public void showChild(View parent, View child, int oldVisibility)
        {
            addChild(parent, child, oldVisibility == View.GONE);
        }

        /**
         * This method is called by View when a child view is about to be removed from the
         * container. This callback starts the process of a transition; we grab the starting
         * values, listen for changes to all of the children of the container, and start appropriate
         * animations.
         *
         ...
         * affect CHANGE_APPEARING or CHANGE_DISAPPEARING animations.
         */
        private void removeChild(View parent, View child, bool changesLayout)
        {
            if (parent.getWindowVisibility() != View.VISIBLE)
            {
                return;
        ...
        /**
         * @deprecated Use {@link #hideChild(android.view.View, android.view.View, int)}.
         */
        [Obsolete]
        public void hideChild(View parent, View child)
        {
            removeChild(parent, child, true);
        }

        /**
         * This method is called by View when a child view is about to be hidden in
         * container. This callback starts the process of a transition; we grab the starting
         * values, listen for changes to all of the children of the container, and start appropriate
         * animations.
         *
         * @param parent The parent View of the View being hidden.
         * @param child The View being hidden.
         * @param newVisibility The new visibility value of the child View, either
         * {@link View#GONE} or {@link View#INVISIBLE}.
         */
        public void hideChild(View parent, View child, int newVisibility)
        {
        ...
        public void addTransitionListener(TransitionListener listener)
        {
            if (mListeners == null)
            {
                mListeners = new List<TransitionListener>();
            }
        ...
        public interface TransitionListener
        {
             ...
             * @param container The View on which the transition is playing.
         ...
         */
        private class CleanupCallback : ViewTreeObserver.OnPreDrawListener,
                View.OnAttachStateChangeListener {

            Dictionary<View, View.OnLayoutChangeListener> layoutChangeListenerMap;
            View parent;

            internal CleanupCallback(Dictionary<View, View.OnLayoutChangeListener> listenerMap, View parent) {
            ...
            private void cleanup()
            {
                ...
                int count = layoutChangeListenerMap.Count();
                if (count > 0)
                {
                    var views = layoutChangeListenerMap.Keys;
                    foreach (View view in views) {
                        View.OnLayoutChangeListener listener = layoutChangeListenerMap[view];
