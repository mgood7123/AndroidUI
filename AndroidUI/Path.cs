...
namespace AndroidUI
{
     ...
     * @hide
     */
    internal readonly SkiaSharp.SKPath mNativePath;
        ...
        public Path()
        {
            mNativePath = new SkiaSharp.SKPath();
        }
...
private static native bool nOp(long path1, long path2, int op, long result);
private static native float[] nApproximate(long nPath, float error);

// ------------------ Fast JNI ------------------------

@FastNative
    private static native bool nIsRect(long nPath, RectF rect);
...
@CriticalNative
    private static native bool nIsEmpty(long nPath);
@CriticalNative
    private static native bool nIsConvex(long nPath);
@CriticalNative
